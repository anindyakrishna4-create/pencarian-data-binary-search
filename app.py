...
# --- Konfigurasi Halaman ---
st.set_page_config(
    page_title="Virtual Lab: Binary Search",
    layout="wide"
)

st.title("✂️ Virtual Lab: Binary Search Interaktif (Matplotlib)")
st.markdown("### Visualisasi Algoritma Pencarian Cepat (Logaritmik)")

st.sidebar.header("Konfigurasi Data dan Target")

# --- Input Data (Tanpa Batas Input) ---
default_data = "12, 45, 90, 3, 55, 18, 70, 25, 60" # Array akan diurutkan secara internal
...
try:
    ...
    # Penting: Tampilkan array yang sudah diurutkan (prasyarat Binary Search)
...
st.markdown("""
#### Pewarnaan Bar:
* **Hijau:** Indeks **Tengah (Mid)** yang sedang dicek.
...
def plot_array(arr, target, low, high, mid, found_index, max_val, status):
    ...
    for i in range(n):
        if low != -1 and high != -1 and low <= i <= high:
             colors[i] = '#F1C232'
    
    # 2. Hijau (#6AA84F): Mid
    if mid != -1 and low <= high:
        colors[mid] = '#6AA84F'

    # 3. Ungu (#8E44AD): Ditemukan
    if found_index != -1:
        colors[found_index] = '#8E44AD'
        # Reset warna mid dan rentang jika sudah ditemukan
    ...
    # Menambahkan label low, high, mid
    if status != 'Selesai' and low <= high:
        ...
        if mid != -1:
             ax.text(mid, max_val * 0.95, f'MID ({mid})', color='green', ha='center', fontsize=10, weight='bold')
...
# --- Visualisasi Utama ---
if st.button("Mulai Simulasi Binary Search"):
    
    found_index, history = binary_search(list(data_list), target_value)
    ...
    st.info(f"Algoritma Binary Search selesai dalam **{len(history)-1}** langkah pengecekan.")
